...
- (void)saveCurrentStillImage
{
    ...
    MingleloopAppDelegate *appdelegate = (MingleloopAppDelegate*)[UIApplication sharedApplication].delegate;
    UIImage *image = [[self.filter imageFromCurrentlyProcessedOutput] imageRotatedByDegrees : 0];
    appdelegate.MediaData=UIImageJPEGRepresentation(image, 1.0);
    //appdelegate.MediaData = imgData;
...
- (id)initWithSessionPreset:(NSString *)sessionPreset cameraPosition:(AVCaptureDevicePosition)cameraPosition highVideoQuality:(BOOL)isHighQuality
{
    ...
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone)
    {
        ...
        {
            gpuImageView = [[GPUImageView alloc] initWithFrame:CGRectMake(10, 25, 300, 300)];
        }
        else
        {
            gpuImageView = [[GPUImageView alloc] initWithFrame:CGRectMake(10, 75, 300, 300)];
        }
    }
