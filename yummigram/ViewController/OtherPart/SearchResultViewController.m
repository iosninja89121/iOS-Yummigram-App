...
- (void) onGridView:(id)sender {
    ...
    [self.collectionViewSearch setHidden:NO];
...
- (void) onListView:(id)sender {
    ...
    [self.collectionViewSearch setHidden:YES];
...
- (void)viewDidLoad {
    ...
    self.arrData = [DataStore instance].wallImagesForSearch;
    
    self.isGridView = NO;
    
    if(self.arrData.count == 0){
        g_lastImageUpdateForSearch = [NSDate date];
     
    }else{
        WallImage *wallImage = [self.arrData firstObject];
        
        g_lastImageUpdateForSearch = wallImage.createdDate;
    }
    
    if(self.nMode == 0){
        [AppDelegate getWallImageForSearchOfNewsFeed:self searchText:self.strSearch limit:LIMIT_NUMBER_GRID];
    }else if(self.nMode == 1){
        [AppDelegate getWallImageForSearchOfRecipe:self searchText:self.strSearch limit:LIMIT_NUMBER_GRID];
    }else if(self.nMode == 2){
        [AppDelegate getWallImageForSearchOfFavorite:self searchText:self.strSearch limit:LIMIT_NUMBER_GRID];
    }
    
    self.lblHeaderTitle.text =[NSString stringWithFormat:@"Search: %@", self.strSearch];
...
-(void)setupCollectionView {
    ...
    [self.collectionViewSearch setPagingEnabled:YES];
    [self.collectionViewSearch setCollectionViewLayout:collectionFlowLayout];
}
...
- (void) initializeRefreshControlForHeader{
    ...
    [self.tblSearch addSubview:self.pullToRefreshView];
}

- (void) initializeRefreshControlForFooter{
    self.indicatorFooter = [[UIActivityIndicatorView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.tblSearch.frame), 44)];
    [self.indicatorFooter setColor:[UIColor blackColor]];
    [self.indicatorFooter stopAnimating];
    [self.tblSearch setTableFooterView:self.indicatorFooter];
...
- (void) loadNextData{
    NSInteger nLimit = (self.isGridView) ? LIMIT_NUMBER_GRID : LIMIT_NUMBER_LIST;
    
    if(self.nMode == 0){
        [AppDelegate getWallImageForSearchOfNewsFeed:self searchText:self.strSearch limit:nLimit];
    }else if(self.nMode == 1){
        [AppDelegate getWallImageForSearchOfRecipe:self searchText:self.strSearch limit:nLimit];
    }else if(self.nMode == 2){
        [AppDelegate getWallImageForSearchOfFavorite:self searchText:self.strSearch limit:nLimit];
    }
}
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    if(self.arrData.count > 0){
        self.collectionViewSearch.backgroundView = nil;
    }
    else{
        ...
        self.collectionViewSearch.backgroundView = messageLabel;
    }
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
    ...
    if (num > 0) {
        ...
        self.tblSearch.separatorStyle = UITableViewCellSeparatorStyleSingleLine;
        ...
        self.tblSearch.backgroundView = messageLabel;
        self.tblSearch.separatorStyle = UITableViewCellSeparatorStyleNone;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UITableViewCell *cell = [self.tblSearch dequeueReusableCellWithIdentifier:WALL_IMAGE_CELL forIndexPath:indexPath];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    UITableViewCell *cell = [self.tblSearch dequeueReusableCellWithIdentifier:WALL_IMAGE_CELL];
